...
import { columnExperience, hastagExperience,textHastag } from '../../../theme/background/Background'
        ...
        <Grid templateColumns="repeat(5,1fr)" justifyItems="center">
            {ArraChartProfile.map((item,index)=>(
                <GridItem w="full" key={item.id}>
                    <Box px="3" position="relative" _before={item.id !== "1" ? columnExperience : {}}>
                        <Text onMouseEnter={()=>openTag(item.id)} onMouseLeave={()=>setTrueTag(false)} textAlign="center" fontSize="13" color="grey.800" cursor="pointer" _hover={{color:"primary_color"}}>{item.experienceHeading}</Text>
                        {(item.id === isShowTag && trueTag) && (
                            <Text sx={textHastag} _before={hastagExperience}>{dataTag[index].tag}</Text>
                        )}
                    </Box>
                </GridItem>
            ))}
