  ...
  const rejectToast = () => {
    ...
    toast({
      ...
      render: () => (
        <Toast
          toastType="reject"
          icon={<Image src={Reject} boxSize="8" />}
          title="You have rejectd"
        >
          John
        </Toast>
      ),
  ...
  const acceptToast = () => {
    ...
    toast({
      ...
      render: () => (
        <Toast
          toastType="accept"
          icon={<Image src={Accpet} boxSize="8" />}
          title="You have just send request for"
        >
          Alaica
        </Toast>
      ),
  ...
  return (
    <Flex zIndex={"docked"} align="center" gap="20">
      ...
      {isDisable ? (
        ...
        ArrayListInformation.filter((item) => item.id === nextSlide).map(
          (item) => <BoxImage list={item} key={item.id} />,
        )
      )}
